...
if __name__ == "__main__":
    ...
    print(f"\n📁 GeoPackage layers (3 essential only):")
    print(f"   🎯 staging_points: Main staging locations + summary stats")
    print(f"   🔍 individual_visibility_zones: Each sample point's visibility (20 per staging)")
    print(f"   🗺️  combined_visibility_zones: Mobility-enhanced optimal zones")

    print(f"\n💡 Simplified Data Structure:")
    print(f"   🔗 staging_id: 1, 2, 3, ... (main staging areas)")
    print(f"   🎯 point_id: 0, 1, 2, ..., 19 (0=center, others=strategic/random)")
    print(f"   📊 staging_point_id: '1_0', '1_1', ... (individual zones)")
    print(f"   📈 No sample point layer, no ray CSV - clean and focused!")

    print(f"\n🗺️  GIS Analysis Examples:")
    print(f"   Individual zones by staging: staging_id = 1")
    print(f"   Center point only: staging_id = 1 AND point_id = 0")
    print(f"   Compare point types: point_type IN ('center', 'ring_1', 'ring_2')")
    print(f"   Large visibility areas: visibility_area_ha > 150")
    print(f"   Combined vs individual: Use both layers to compare")
    print(f"   High improvement staging: area_improvement_pct > 50")

    print(f"\n🎯 Key Improvements:")
    print(f"   ✅ Cleaner, more efficient code structure")
    print(f"   ✅ Both strategic (radial) and random sampling options")
    print(f"   ✅ Individual visibility zones for each sample point")
    print(f"   ✅ Complete tracking with staging_id + point_id + ray_id")
    print(f"   ✅ Proper error handling and verbose output")
    print(f"   ✅ Ready for advanced GIS analysis and filtering")